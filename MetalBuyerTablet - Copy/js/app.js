...
async function captureAndReadScale() {
  ...
  try {
    const result = await Tesseract.recognize(canvas, "eng", {
      tessedit_char_whitelist: "0123456789.,gGoOzZtT"
    });
    const text = result.data.text.trim();
    const parsed = parseWeight(text);
    $("ocrOutput").textContent = text || "No text found. Try closer, brighter, and straighter.";
    if (parsed.value > 0) {
...
function parseWeight(text) {
  ...
  const match = normalized.match(/(\d+(?:\.\d+)?)\s*(ozt|troy\s*oz|oz|ounces?|grams?|g)?/);
  const value = match ? parseFloat(match[1]) : 0;
  const unitText = match?.[2] || "";
  const unit = /ozt|troy/.test(unitText) ? "ozt" : /oz|ounce/.test(unitText) ? "oz" : "g";
